...
const DEFAULT_DESCRIPTION =
  "Details volgen zodra deze collectable volledig is verwerkt.";
...
export const validateProductsJson = (rawProducts) => {
  ...
  rawProducts.forEach((product, index) => {
    ...
    if (
      ...
      warnings.push(
        `Product at index ${index} has an invalid images value. Expected string[] or string.`
      );
    }
    ...
    if (
      ...
      warnings.push(
        `Product at index ${index} has an invalid gallery value. Expected string[] or string.`
      );
    }
  });
...
export const normalizeProduct = (rawProduct = {}) => {
  ...
  normalized.purchasePrice =
    Number(rawProduct.purchasePrice) || 0;

  normalized.sellingPrice =
    getSellingPrice(rawProduct);
  ...
  normalized.stock =
    Number(rawProduct.stock) || 0;

  normalized.reserved =
    Number(rawProduct.reserved) || 0;

  const releaseYear =
    Number(rawProduct.releaseYear);
  ...
  normalized.exclusive =
    toBoolean(rawProduct.exclusive);

  normalized.chase =
    toBoolean(rawProduct.chase);

  normalized.vaulted =
    toBoolean(rawProduct.vaulted);

  normalized.signed =
    toBoolean(rawProduct.signed);

  normalized.neverOutOfBox =
    toBoolean(rawProduct.neverOutOfBox);

  normalized.figureLikeNew =
    toBoolean(rawProduct.figureLikeNew);
  ...
  normalized.metaTitle =
    rawProduct.metaTitle ||
    `${rawProduct.name || "Collectible"} | 2nd Life Toys`;

  normalized.metaDescription =
    rawProduct.metaDescription ||
  ...
  normalized.description =
    (
      ...
      normalized.metaDescription ||
      ""
    ).trim() || DEFAULT_DESCRIPTION;

  normalized.tags =
    Array.isArray(rawProduct.tags)
      ? rawProduct.tags
...
export const normalizeProductCatalog = (
  rawProducts = []
) => {
  const validation =
    validateProductsJson(rawProducts);

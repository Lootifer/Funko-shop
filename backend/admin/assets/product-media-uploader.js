...
const uploadMappedFiles = async (mapped = []) => {
  ...
  try {
    ...
    // V20.8: preserve non-replaced slots when editing an existing product,
    // while replacing placeholder/old files for the slots that were uploaded.
    const existingPaths = String(fieldImages?.value || "")
      .split(/\r?\n|\|/)
      .map((value) => value.trim())
      .filter((value) => value && !/premium-placeholder\.svg$/i.test(value));

    const existingBySlot = Object.fromEntries(SLOT_ORDER.map((slot) => [
      slot,
      existingPaths.find((value) => new RegExp(`/${slot}\\.(?:jpe?g|png|webp)$`, "i").test(value)) || "",
    ]));
    const uploadedBySlot = Object.fromEntries(preparedMapped.map((entry, index) => [entry.slot, uploadedPaths[index] || ""]));
    const finalPaths = SLOT_ORDER
      .map((slot) => uploadedBySlot[slot] || existingBySlot[slot] || "")
      .filter(Boolean);

    if (fieldImages) {
      fieldImages.value = finalPaths.join("\n");
      fieldImages.dispatchEvent(new Event("input", { bubbles: true }));
      fieldImages.dispatchEvent(new Event("change", { bubbles: true }));
    }

    // Tell the product admin that media changed. When an existing product is
    // being edited, products-admin.js stores these paths in the database immediately.
    window.dispatchEvent(new CustomEvent("lootifer:product-media-uploaded", {
      detail: { slug, images: finalPaths },
    }));
